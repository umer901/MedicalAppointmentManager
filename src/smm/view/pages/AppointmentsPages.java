...
/** Appointments pages (List, Create, Details) */
public class AppointmentsPages {

    /* ===========================
       List Page (with Details)
       =========================== */
    public static class ListPage extends NavAwarePanel {
        ...
        private final DefaultTableModel model = new DefaultTableModel(
            ...
            @Override public Class<?> getColumnClass(int c){
                return switch (c){
                    case 0 -> UUID.class;
                    case 1 -> java.time.LocalDate.class;
                    case 2 -> java.time.LocalTime.class;
                    case 7 -> String.class;
                    case 8 -> Double.class;
        ...
        private void paySelected() {
            ...
            // If pricing is Deferred, disallow paying now
            if (c.getModel().canDeferredPayment()) {
                JOptionPane.showMessageDialog(this, "Payment is deferred under current Pricing.");
                return;
            }
            ...
            double amount = c.getModel().priceAfterPlan(a.price);
            ...
            for (Appointment a : list) {
                ...
                if (okType && okDoc && okDate) {
                    model.addRow(new Object[]{
                            ...
                            a.paid ? "Yes" : "No", M.priceAfterPlan(a.price)
                    });
                }
            }
        }
        ...
        private class DetailsDialog extends JDialog {
            ...
            private void fill() {
                ...
                sb.append(a.summary()).append("\n\n")
                  ...
                  .append("After insurance/pricing: ").append(m.priceAfterPlan(a.price)).append("€\n")
                  .append("Pricing: ").append(m.currentPricing()).append(" | Policy: ").append(m.currentPolicy()).append("\n")
    ...
    public static class CreatePage extends NavAwarePanel {
        ...
        private void updateComputedPrice() {
            ...
            double afterPlan = c.getModel().priceAfterPlan(base);

            if (c.getModel().canDeferredPayment()) {
                lbComputed.setText(String.format(
                        "You pay now: 0.00€  (Deferred: %.2f€)  [Policy: %s, Pricing: %s]",
                        afterPlan, c.getModel().currentPolicy(), c.getModel().currentPricing()
                ));
            } else {
                lbComputed.setText(String.format(
                        "You pay: %.2f€  [Policy: %s, Pricing: %s]",
                        afterPlan, c.getModel().currentPolicy(), c.getModel().currentPricing()
                ));
            }
        }
        ...
        private void onCreate() {
            ...
            boolean payNowEffective = chkPayNow.isSelected();
            if (c.getModel().canDeferredPayment()) payNowEffective = false;
            ...
            var a = c.createBooking(
                ...
                null, null,
                base,
                chkCalendar.isSelected(), payNowEffective
            );

            double due = c.getModel().priceAfterPlan(a.price);
            String msg = c.getModel().canDeferredPayment()
                    ? String.format("Created:\n%s\nYou pay now: 0.00€ (Deferred: %.2f€).",
                                    a.summary(), due)
                    : String.format("Created:\n%s\nYou pay now: %.2f€.",
                                    a.summary(), due);
        ...
        @Override public void refresh() {
            updateComputedPrice(); // recompute in case insurance/pricing changed
        }
    }
        ...
        @Override public void refresh() {
            ...
            if (a == null) {
                ...
                for (Appointment ap : m.appointments) {
                    sb.append("• ").append(ap.summary()).append(" | After insurance/pricing: ")
                      .append(m.priceAfterPlan(ap.price)).append("€ | Paid=").append(ap.paid).append("\n");
                }
            } else {
                sb.append(a.summary()).append("\n\n")
                  .append("Base price: ").append(a.price).append("€\n")
